...
async def admin_payments(message: Message):
    ...
    text = (
        "💰 Платежи\n\n"
        f"Сегодня: {stats['today_amount']} ₽ / {stats['today_count']} платеж(ей) / {stats['today_spreads']} расклад(ов)\n"
        f"Всего: {stats['total_amount']} ₽ / {stats['total_count']} платеж(ей) / {stats['total_spreads']} расклад(ов)\n\n"
    )

    if not payments:
        await message.answer(text + "Платежей пока нет.")
        return

    text += "Последние платежи:\n\n"

    for p in payments:
        username = p["username"] or "без username"
        first_name = p["first_name"] or "без имени"
        text += (
            f"#{p['id']} — {p['amount']} ₽\n"
            f"Пользователь: {first_name} (@{username})\n"
            f"ID: {p['user_id']}\n"
            f"Раскладов: {p['spreads_added']}\n"
            f"Дата: {p['created_at']}\n\n"
        )
